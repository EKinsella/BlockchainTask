...
class App extends Component {
  ...
  async loadBlockchainData(dispatch) {
    ...
    if (typeof window.ethereum !== 'undefined') {
      ...
      {
        window.alert("Error getting account details");
      }
      //assign to values to variables: web3, netId, accounts
      const web3 = new Web3(window.ethereum)
      const netId = await web3.eth.netId()
      ...
      try {
        ...
        const dBankAddress = dBankAddress.networks[netId].address
  ...
  async borrow(amount) {
    if(this.state.dbank!=='undefined'){
      try{
        await this.state.dbank.methods.borrow().send({value: this.account.toString, from: this.state.account})
      } catch (err) {
        console.log('An error occurred', err)
      }
    }
